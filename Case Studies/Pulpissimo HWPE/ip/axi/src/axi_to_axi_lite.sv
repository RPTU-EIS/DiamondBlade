...
module axi_to_axi_lite #(
 ...
 `ifndef VERILATOR
  initial begin
    assert (AxiIdWidth   > 0) else $fatal(1, "AXI ID width has to be > 0");
    assert (AxiAddrWidth > 0) else $fatal(1, "AXI address width has to be > 0");
    assert (AxiDataWidth > 0) else $fatal(1, "AXI data width has to be > 0");
  end
  `endif
  // pragma translate_on
endmodule
